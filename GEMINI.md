...
- **Phase 3 (완료)**: 멀티팩터 스코어링(Z-Score), 섹터 로테이션, 동적 자산배분(듀얼 모멘텀), 리스크 패리티 등 고도화된 퀀트 전략 백테스팅 엔진 구현 완성 (2026-02-24 완료).

## 최근 변경사항 및 수정 (2026-02-24)

### 2. 고급 퀀트 전략 고도화 (Phase 3 완료) ✅
**목적**: 단순 조건 필터링을 넘어 통계적 방법론과 포트폴리오 자산 배분/위험 관리 로직 적용

**주요 구현 사항**:
- **멀티팩터 스코어링 (Z-Score)**: `FactorScoringService`를 통해 단위가 다른 지표(PER, PBR, ROE, 6M Momentum 등)를 평균과 표준편차를 활용해 정규화(Z-Score)하고, 설정된 비중에 따라 합산 순위를 매겨 포트폴리오 구성.
- **섹터 로테이션 전략**: 12개월 모멘텀이 높은 상위 섹터(업종)를 선별 후, 해당 섹터 내 우량/강세 주식으로 자금을 순환 매수하는 로직 구현 (`SectorAnalysisService`).
- **동적 자산배분 (듀얼 모멘텀)**: 시장 전체(유니버스) 평균 모멘텀이 마이너스로 돌아설 때, 위험자산(주식) 투자 비중을 0%로 줄이고 현금성 안전자산으로 회피하는 로직 적용.
- **리스크 패리티 (Inverse Volatility)**: 단일 자산의 변동성이 포트폴리오 전체에 미치는 영향을 균등화하기 위해, 각 종목의 변동성(볼린저 밴드 대역폭 기반)의 역수에 비례하여 자본을 배분 (`AssetAllocationStrategy`).
- **DB 업데이트**: 섹터별 모멘텀/상대강도를 기록하는 `TB_SECTOR_ANALYSIS` 및 일별 종목별 팩터 점수를 저장하는 `TB_FACTOR_SCORE` 테이블 추가 (Flyway `V3__phase3_advanced_strategies.sql`).
