        ...
        if (StringUtils.hasText(xml)) {
            ...
            for (int i = 0; i < itemList.getLength(); i++) {
                ...
                priceList.add(StockPrice.builder()
                        ...
                        .volume(safeParseLong(getTagValue("trqu", item)))
                        .volumePrice(safeParseLong(getTagValue("trPrc", item)))
                        .startPrice(safeParseLong(getTagValue("mkp", item)))
                        .endPrice(safeParseLong(getTagValue("clpr", item)))
                        .highPrice(safeParseLong(getTagValue("hipr", item)))
                        .lowPrice(safeParseLong(getTagValue("lopr", item)))
